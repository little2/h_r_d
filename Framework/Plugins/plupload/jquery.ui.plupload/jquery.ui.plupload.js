					...
					"ui.plupload",
					{
						...
						_create : function() {
							...
							j.bind("UploadProgress", function(l, m) {
								g("#" + m.id).find(".plupload_file_status").html(m.percent + "%").end().find(
										".plupload_file_size").html(c.formatSize(m.size));
								i._handleFileStatus(m);
												...
												if (n) {
													...
													if (m) {
														...
														case c.FILE_SIZE_ERROR:
															m = b("File: %f, size: %s, max file size: %m").replace(
																	/%([fsm])/g, function(r, q) {
																		switch (q) {
																		case "f":
																			return n.name;
						...
						_handleState : function() {
							...
							if (i.state === c.STARTED) {
								...
								g(".plupload_upload_status", j.element).text(
										b("Uploaded %d/%d files").replace("%d/%d",
						...
						_handleFileStatus : function(l) {
							...
							case c.UPLOADING:
								...
								var i = g(".plupload_scroll", this.container), m = i.scrollTop(), o = i.height(), k = g(
										"#" + l.id).position().top
										+ g("#" + l.id).height();
								if (o < k) {
									i.scrollTop(m + k - o)
						...
						_updateTotalProgress : function() {
							...
							this.element.find(".plupload_total_status").html(i.total.percent + "%").end().find(
									".plupload_total_file_size").html(c.formatSize(i.total.size)).end().find(
						...
						_updateFileList : function() {
							...
							g
									.each(
											...
											function(q, p) {
												...
												if (p.status === c.DONE) {
													if (p.target_name) {
														i += '<input type="hidden" name="' + o + '_tmpname" value="'
												...
												g(
														"#" + p.id + ".plupload_delete .ui-icon, #" + p.id
							...
							if (j.total.queued === 0) {
								...
								g(".ui-button-text", k.browse_button).text(
										b("%d files queued").replace("%d", j.total.queued))
							}
							...
							if (!j.files.length && j.features.dragdrop && j.settings.dragdrop) {
								g("#" + o + "_filelist").append(
										'<tr><td class="plupload_droptext">' + b("Drag files here.") + "</td></tr>")
							} else {
						...
						_enableDragAndDrop : function() {
							this.filelist.append('<tr><td class="plupload_droptext">' + b("Drag files here.")
									+ "</td></tr>");
							...
							g("tbody", this.filelist).sortable(
									{
										...
										helper : function(l, k) {
											return k.clone(true).find("td:not(.plupload_file_name)").remove().end()
													.css("width", "100%")
										},
										stop : function(p, o) {
											var l, n, k, m = [];
						...
						notify : function(j, k) {
							var i = g('<div class="plupload_message"><span class="plupload_message_close ui-icon ui-icon-circle-close" title="'
									+ b("Close") + '"></span><p><span class="ui-icon"></span>' + k + "</p></div>");
							i.addClass("ui-state-" + (j === "error" ? "error" : "highlight")).find("p .ui-icon")
									.addClass("ui-icon-" + (j === "error" ? "alert" : "info")).end().find(
