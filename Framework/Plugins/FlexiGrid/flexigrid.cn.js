...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			onRowClick : false,
			onRowDblclick:false//rowdbclick自定义函数
		  }, p);
		...
		var g = {
			...
			dragStart : function(dragtype, e, obj) { // default drag function start
					...
					else if (dragtype == 'colMove' && p.dragCol) // column header drag
					{
						$(g.nDiv).hide();
						$(g.nBtn).hide();
				...
				if (p.dataType == 'json') {
					$.each(data.page.list, function(i, row) {
						    ...
						    $('thead tr:first th', g.hDiv).each(function() {
							        ...
							        if(cm.type){
							        	var input = $('<input/>');
							        	if(cm.type) input.attr("type",cm.type);
							        	if(cm.bclass) input.addClass(cm.bclass);
							        	$(td).append(input);
							        }else{
						 ...
						 // 添加多选
						    if (p.chooseType=='radio') {
							    var cth = $('<th/>');
                            var cthch = $('<input type="radio" value="'
							        + $(tr).attr('id') + '"/>');
							    cth.addClass("cth").attr({
								        width : "22"
							        }).append(cthch);
			...
			addRowProp : function() {
				$('tbody tr', g.bDiv).each(function() {
					    $(this).dblclick(function (e) {   
				    	        ...
				    	        if (p.onRowDblclick) p.onRowDblclick($(rowData),p.expend,g.gDiv);   
					    	 }).click(function(e) {
						        var obj = (e.target || e.srcElement);
						        if (obj.href || obj.type){
						        	...
						        	if(axis){
						        		var idx =axis.substr(3);
							        	 ...
							        	 var cm = p.colModel[idx];
							        	 cm.onpress(p.expend,$(rowData),obj);
							        	 return true;
						        	}
						        }
						        $(this).toggleClass('trSelected');
						        ...
						        if (p.onRowClick) p.onRowClick($(rowData),p.expend,g.gDiv); 
		...
		if (p.usepager) {
			...
			if (p.useRp) {
				...
				$('.pcoadright', g.pDiv).prepend("<div class='pGroup'><select name='rp' class='pcoadbh'>"
