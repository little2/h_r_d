...
(function($) {
	$.fn.smartWizard = function(action) {
		...
		return this.each(function() {
			...
			var curSelectedStepIdx = options.selected;
			var steps = $("ul > li > a[href^='#step-']", obj); // Get all
			// anchors in
			// this array
			var contentWidth = 0;
			...
			function init() {
				...
				$(steps).bind("click", function(e) {
					if (steps.index(this) == curSelectedStepIdx) {
						return false;
					}
					// var nextStepIdx = steps.index(this);
					var nextSelectedStepIdx = steps.index(this);
					var isDone = steps.eq(nextSelectedStepIdx).attr("isDone") - 0;
					if (isDone != 0) {
						LoadContent(nextSelectedStepIdx, 'click');
					}
					return false;
				});
				...
				LoadContent(curSelectedStepIdx);
			}
			...
			function LoadContent(stepIdx, type) {
			...
			function showStep(stepIdx, type) {
				var selStep = steps.eq(stepIdx);
				var curStep = steps.eq(curStepIdx);
				if (type) {
					curStep = steps.eq(curSelectedStepIdx);
					...
					if (options.transitionEffect == 'slide') {
						$($(curStep, obj).attr("href"), obj).slideUp("fast", function(e) {
							...
							curSelectedStepIdx = stepIdx;
							SelectedStep(curStep, selStep);
						});
			...
			function SelectedStep(curStep, selStep) {
				$(curStep, obj).removeClass("selected");
				$(selStep, obj).addClass("selected");
			}
