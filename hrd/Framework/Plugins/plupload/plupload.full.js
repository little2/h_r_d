						...
						g.each(H, function(K) {
							g.each(K.extensions.split(/,/), function(L) {
								if (/^\s*\*\s*$/.test(L)) {
									...
									J.push("\\."
											+ L.replace(new RegExp("[" + ("/^$.*+?|()[]{}\\".replace(/./g, "\\$&"))
													+ "]", "g"), "\\$&"))
								}
							})
						});
				...
				function p() {
					...
					function A(E) {
						...
						if (y) {
							...
							D.append(G + J + I + 'Content-Disposition: form-data; name="' + u.settings.file_data_name
									+ '"; filename="' + r.name + '"' + I + "Content-Type: " + H + I + I);
							D.append(E);
							D.append(I + G + J + G + I);
							F = D.getAsBlob();
							x = F.length - E.length;
					...
					"silverlight",
					{
						...
						init : function(p, q) {
							...
							o.innerHTML = '<object id="'
									...
									+ p.id + ",filter=" + m + ",multiselect=" + p.settings.multi_selection
									+ '"/></object>';
							function j() {
							...
							p
									.bind(
											...
											function(s, u) {
													...
													function B() {
														...
														function G(V) {
															...
															if (s.settings.multipart && n.multipart) {
																...
																if (typeof (V) === "string") {
																	q.setRequestHeader("Content-Type",
															...
															E = j.buildUrl(s.settings.url, j.extend(L,
																	s.settings.multipart_params));
															q.open("post", E, true);
															q.setRequestHeader("Content-Type",
																	"application/octet-stream");
														...
														if (u.status == j.DONE || u.status == j.FAILED
																|| s.state == j.STOPPED) {
															return
														}
														L = {
														...
														if (v.chunk_size && u.size > v.chunk_size
														...
														if (s.settings.multipart && n.multipart
																&& typeof (I) !== "string" && z
																&& n.cantSendBlobInFormData && n.chunks
												...
												y = c[u.id];
												if (n.jpgresize && s.settings.resize
														&& /\.(png|jpg|jpeg)$/i.test(u.name)) {
													d.call(s, u, s.settings.resize,
					...
					.bind("Init",
							function(p) {
								...
								function u() {
									...
									y.innerHTML = '<iframe id="' + p.id + '_iframe" name="' + p.id + '_iframe" src="'
									...
									b.addEvent(n, "load", function(D) {
										...
										try {
											C = E.contentWindow.document || E.contentDocument
