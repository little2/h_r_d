					...
					.each(function() {
						...
						function g() {
									...
									.each(
											...
											function(p, o) {
												...
												if (o.status == plupload.DONE) {
													...
													l += '<input type="hidden" name="' + k + "_" + n + '_name" value="'
												...
												m
														.append('<li id="'
																...
																+ o.percent + '%</div><div class="plupload_file_size">'
							...
							if (!j.files.length && j.features.dragdrop && j.settings.dragdrop) {
								c("#" + k + "_filelist").append(
										'<li class="plupload_droptext">' + a("Drag files here.") + "</li>")
							}
						}
						...
						j.bind("Error", function(l, o) {
							...
							if (m) {
								...
								m.hint = n;
								c("#" + m.id).attr("class", "plupload_failed").find("a").css("display", "block").attr(
										"title", n)
							}
						});
						j.bind("StateChanged", function() {
							if (j.state === plupload.STARTED) {
								...
								c("span.plupload_upload_status,div.plupload_progress,a.plupload_stop", i).css(
										"display", "block");
								c("span.plupload_upload_status", i).text(
										"Uploaded " + j.total.uploaded + "/" + j.files.length + " files");
